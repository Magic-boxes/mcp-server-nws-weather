...
class MCPClient:
    ...
    async def connect_to_sse_server(self, server_url: str):
        ...
        # 关键修改：使用 exit_stack 来管理连接，不要手动调用 __aenter__
    ...
    async def process_query(self, query: str, url: str) -> str:
        ...
        if not self._is_connected:
            raise RuntimeError("请先调用 connect_to_sse_server 连接服务器")
...
"""
        ...
        if think_match:
            ...
            final_answer = re.sub(think_pattern, '', res, flags=re.DOTALL).strip()
        else:
            # 没有 think 标签的情况
            thinking_content = ""
            final_answer = res.strip()  # 直接使用整个响应作为最终答案

        print("LLM模型think内容：", thinking_content)
        print("LLM模型最终结果：", final_answer)
